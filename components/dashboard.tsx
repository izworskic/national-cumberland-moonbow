  ...
  return <>
    <div className="main-content shell">
      ...
      <div className="dashboard-grid">
        <section className="data-card" aria-labelledby="moon-title"><div className="card-title"><MoonIcon /><div><p className="kicker">Deterministic</p><h2 id="moon-title">Moon</h2></div></div>
          <div className="metric-row"><div><span>Illumination</span><strong>{percent(result.astronomySummary.illumination)}</strong></div><div><span>Peak altitude</span><strong>{peakPoint ? `${round(peakPoint.astronomy.moonAltitude, 1)}°` : "—"}</strong></div><div><span>Azimuth</span><strong>{peakPoint ? `${Math.round(peakPoint.astronomy.moonAzimuth)}°` : "—"}</strong></div></div>
          ...
          <p className="card-note">Effective skyline {peakPoint?.astronomy.effectiveSkyline ?? "—"}°: {peakPoint?.astronomy.terrainHorizon ?? "—"}° bare earth + {peakPoint?.astronomy.treeCorrection ?? "—"}° trees/clearance.</p>
          <SourceStamp source={result.sources.find((source) => source.id === "astronomy-engine")!} />
        </section>

        <section className="data-card data-card--cloud" aria-labelledby="cloud-title"><div className="card-title"><CloudIcon /><div><p className="kicker">{planning ? "Seasonal history" : "Forecast + nowcast"}</p><h2 id="cloud-title">Clouds</h2></div></div>
          {planning ? <>
            <div className="metric-row"><div><span>Expected transmission</span><strong>{percent(peakPoint?.factors.cloud)}</strong></div><div><span>NWS forecast</span><strong>Not yet</strong></div><div><span>GOES nowcast</span><strong>Not yet</strong></div></div>
            ...
            <div className="metric-row"><div><span>NWS forecast</span><strong>{percent(forecastCloud)}</strong></div><div><span>GOES mask</span><strong>{result.satellite.classification.replace("PROBABLY ", "PROB. ")}</strong></div><div><span>Trend</span><strong>{result.satellite.trend}</strong></div></div>
            ...
            <p className="card-note">Satellite gains weight only inside the 0–3 hour nowcast. It is an observation, not a forecast.</p>
            <SourceStamp source={result.satellite.source} />
          </>}
        </section>

        <section className="data-card" aria-labelledby="falls-title"><div className="card-title"><RiverIcon /><div><p className="kicker">USGS 03404500</p><h2 id="falls-title">The falls</h2></div></div>
          ...
          <p className="card-note">The V1 mist proxy saturates as flow rises; it does not claim that “more is always better” or invent a narrow ideal range.</p>
          <SourceStamp source={result.hydro.source} />
        </section>

        <section className="data-card" aria-labelledby="why-title"><div className="card-title"><ArrowIcon /><div><p className="kicker">Conjunctive model</p><h2 id="why-title">Why the score</h2></div></div>
          <ul className="factor-list">
            {Object.entries(peakPoint?.factors ?? {}).map(([label, value]) => <li key={label}><span>{label}</span><div><i style={{ width: `${Math.round(value * 100)}%` }} /></div><strong>{Math.round(value * 100)}</strong></li>)}
          </ul>
          <p className="card-note">Factors multiply. A blocked Moon or near-zero cloud transmission collapses the result instead of being averaged away.</p>
        </section>
